...
function sincronizarDadosRecuperacao() {
  ...
  try {
    ...
    const cpfMap = buildCPFMap(usuarios);
    const todosColabs = lerTodosColaboradores();
    if (!todosColabs || todosColabs.length === 0) {
      Logger.json('warn', { event: "TRIGGER_ABORT", reason: "No HCM data" });
      return;
    }

    const colabs = todosColabs.filter(colab => SITUACOES.ATIVAS.has(String(colab.situacao || '')));
    if (!colabs || colabs.length === 0) {
      console.warn({ event: "TRIGGER_ABORT", reason: "No active HCM collaborators" });
      return;
    }

    let sucessos = 0, erros = 0, atualizacoes = 0;
    colabs.forEach(colab => {
      try {
        const usuarioGW = obterUsuarioGW(colab, usuarios, cpfMap);
        if (!usuarioGW) {
          Logger.json('warn', { event: "USER_NOT_FOUND", nome: colab.nomeCompleto || null });
          return;
        }

        const atualizacoesNecessarias = obterAtualizacoesDadosRecuperacao(usuarioGW, colab);
        if (Object.keys(atualizacoesNecessarias).length > 0) {
          atualizarUsuarioComRetry(atualizacoesNecessarias, usuarioGW.primaryEmail, {
            source: "SYNC_RECOVERY_DATA",
            email: usuarioGW.primaryEmail,
            cpf: colab.cpf
          });
          atualizacoes++;
          Logger.json('info', {
            event: "RECOVERY_DATA_UPDATED",
            name: usuarioGW.name && (usuarioGW.name.fullName || (usuarioGW.name.givenName + ' ' + usuarioGW.name.familyName)),
            updates: Object.keys(atualizacoesNecessarias)
          });
        }
        ...
        erros++;
        Logger.json('error', { event: "PROCESS_ERROR", name: colab.nomeCompleto || null, error: e.message });
      }
    });

    Logger.json('info', {
      event: "TRIGGER_RECOVERY_COMPLETE",
      execution_id: execId,
      sucessos: sucessos,
      erros: erros,
    ...
    Logger.json('error', { event: "TRIGGER_FATAL_ERROR", error: e.message });
  }
}

// Funções de atualização para sincronização do Google Workspace com HCM

function obterAtualizacoesDadosRecuperacao(usuarioGW, dadosHCM) {
  try {
    const atualizacoes = {};
    const emailPessoal = (dadosHCM.emailPessoal || '').trim().toLowerCase();
    const emailRecGW = (usuarioGW.recoveryEmail || '').trim().toLowerCase();

    if (saoDiferentes(emailRecGW, emailPessoal)) {
      if (!emailPessoal) {
        atualizacoes.recoveryEmail = null;
        Logger.json('info', { event: 'RECOVERY_EMAIL_REMOVED', name: usuarioGW.name && (usuarioGW.name.fullName || (usuarioGW.name.givenName + ' ' + usuarioGW.name.familyName)) });
      } else if (!emailPessoal.includes(NOME_DOMINIO.toLowerCase())) {
        atualizacoes.recoveryEmail = dadosHCM.emailPessoal.trim();
        Logger.json('info', { event: 'RECOVERY_EMAIL_UPDATED', name: usuarioGW.name && (usuarioGW.name.fullName || (usuarioGW.name.givenName + ' ' + usuarioGW.name.familyName)) });
      } else {
        Logger.json('warn', { event: 'RECOVERY_EMAIL_IGNORED_COMPANY_DOMAIN', name: usuarioGW.name && (usuarioGW.name.fullName || (usuarioGW.name.givenName + ' ' + usuarioGW.name.familyName)) });
      }
    } else {
      Logger.json('info', { event: 'RECOVERY_EMAIL_NO_CHANGE', name: usuarioGW.name && (usuarioGW.name.fullName || (usuarioGW.name.givenName + ' ' + usuarioGW.name.familyName)) });
    }

    let telHCMNormalizado = null;
    if (dadosHCM.numeroCelular) {
      const telNormalizado = UtilsTelefone.normalizar(dadosHCM.numeroCelular);
      Logger.json('info', { event: 'PHONE_NORMALIZED', name: usuarioGW.name && (usuarioGW.name.fullName || (usuarioGW.name.givenName + ' ' + usuarioGW.name.familyName)) });
      if (telNormalizado && telNormalizado.trim()) {
        telHCMNormalizado = telNormalizado;
      } else if (dadosHCM.numeroCelular.trim()) {
        Logger.json('warn', { event: 'PHONE_INVALID', name: usuarioGW.name && (usuarioGW.name.fullName || (usuarioGW.name.givenName + ' ' + usuarioGW.name.familyName)) });
      }
    }
    const telGW = usuarioGW.recoveryPhone;

    if (saoDiferentes(telGW, telHCMNormalizado)) {
      atualizacoes.recoveryPhone = telHCMNormalizado;
      Logger.json('info', { event: 'RECOVERY_PHONE_UPDATED', name: usuarioGW.name && (usuarioGW.name.fullName || (usuarioGW.name.givenName + ' ' + usuarioGW.name.familyName)) });
    } else {
      Logger.json('info', { event: 'RECOVERY_PHONE_NO_CHANGE', name: usuarioGW.name && (usuarioGW.name.fullName || (usuarioGW.name.givenName + ' ' + usuarioGW.name.familyName)) });
    }

    return atualizacoes;
  } catch (e) {
    Logger.json('error', { evento: "ERRO_DIFERENCA_RECUPERACAO", name: usuarioGW.name && (usuarioGW.name.fullName || (usuarioGW.name.givenName + ' ' + usuarioGW.name.familyName)), erro: e.message });
    return {};
  }
}
...
function atualizarNomeEOU(usuarioGW, dadosHCM, uoDestino) {
  try {
    ...
    if (!needsNameUpdate && !needsOUUpdate) {
      console.info(`[processarColaboradoresNomeOU] Sem alteração: ${mascararEmailParaLog(usuarioGW.primaryEmail)}`);
      return false;
    }
    ...
    console.info(JSON.stringify({ evento: "NOME_OU_ATUALIZADO", email: mascararEmailParaLog(usuarioGW.primaryEmail), dados: payload }));
    return true;
  } catch (e) {
...
function atualizarCustomSchemas(usuarioGW, dadosHCM) {
  try {
    ...
    if (!temAlteracao) {
      console.info('[processarColaboradoresSchemas] Schemas sem alteração: ' + mascararEmailParaLog(usuarioGW.primaryEmail));
      return false;
    }
    ...
    console.info(JSON.stringify({ evento: "SCHEMAS_ATUALIZADOS", email: mascararEmailParaLog(usuarioGW.primaryEmail), campos: Object.keys(customSchemas) }));
    return true;
  } catch (e) {
...
function atualizarOrganizacao(usuarioGW, dadosHCM) {
  try {
    ...
    if (!needsUpdate) {
      console.info('[processarColaboradoresOrganizacao] Sem alteração para: ' + mascararEmailParaLog(usuarioGW.primaryEmail));
      return false;
    }
    const payload = {
      organizations: [{
    ...
    console.info(JSON.stringify({ evento: "ORGANIZACAO_ATUALIZADA", email: mascararEmailParaLog(usuarioGW.primaryEmail) }));
    return true;
  } catch (e) {
