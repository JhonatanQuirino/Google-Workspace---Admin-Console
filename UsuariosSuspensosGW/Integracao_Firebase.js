  ...
  try {
    ...
    const firebase = {
      ...
      request: function(path, method, data, retryCount) {
        ...
        console.log('[DEBUG][FIREBASE] URL final: ' + url);
        const options = {
          method: method,
...
function renovarTokenFirebase() {
  try {
    ...
    if (novoToken) {
      console.log('[renovarTokenFirebase] Token renovado com sucesso');
      return { 
        sucesso: true, 
    ...
    console.error('[renovarTokenFirebase] ERRO: ' + e.message);
    return { 
      sucesso: false, 
...
// =====================================================
// ColaboradorService: Classe de transformação e validação de dados de colaborador
// =====================================================
class ColaboradorService {
  ...
  tratarDados() {
    const resultado = {
      cpf: this.dados.cpf || null,
      emailProfissional: this.dados.emailProfissional || null,
      emailPessoal: this.dados.emailPessoal || null,
      nome: this.dados.nome || this.dados.nomeCompleto || null,
      cargo: this.dados.cargo || null,
      departamento: this.dados.departamento || null,
      centroCusto: this.dados.centroCusto || null,
      dataAdmissao: this.dados.dataAdmissao || null,
      situacao: this.dados.situacao || null,
      manager: this.dados.manager || null,
      telefone: this.dados.telefone || null,
      customSchemas: this.dados.customSchemas || {},
      auditoria: this.dados.auditoria || []
    };
    return resultado;
  }

  adicionarAuditoria(tipo, usuario, detalhes) {
  ...
  try {
    ...
    Object.keys(dados).forEach(function(key) {
      ...
      try {
        ...
        var tratado = colaborador.tratarDados();

        var cpf = tratado && tratado.cpf ? String(tratado.cpf).trim() : '';
        var emailProfissional = tratado && tratado.emailProfissional ? String(tratado.emailProfissional).trim() : '';

        if (!cpf && !emailProfissional) {
          invalidos.push({ key: key, nome: tratado && tratado.nome ? tratado.nome : null });
          return;
        }

        colaboradores.push(tratado);
      } catch (eItem) {
        console.warn('lerTodosColaboradores - colaborador inválido no Firebase', { key: key, error: eItem && eItem.message ? eItem.message : eItem });
      }
    });

    if (invalidos.length > 0) {
      console.warn({
        event: 'LER_COLABORADORES_SEM_IDENTIFICACAO',
        total: invalidos.length,
        samples: invalidos.slice(0, 3)
      });
    }
...
function registrarDadosNoFirebase(dadosTratados) {
  try {
    ...
    // LOG DIAGNÓSTICO: dados tratados antes de escrever no Firebase
    try {
      console.info('[DIAG] registrarDadosNoFirebase - dados tratados:', {
        cpf: dados && dados.cpf ? dados.cpf : null,
        emailProfissional: dados && dados.emailProfissional ? dados.emailProfissional : null,
        nome: dados && dados.nome ? dados.nome : null,
